...
class SnapshotBase(BaseSchema):
    ...
    trigger_type: str = Field(..., description="Trigger type (manual, scheduled, api, cli)")
    triggered_by: Optional[str] = Field(None, description="User or system that triggered snapshot")
...
class SnapshotSummary(SnapshotResponse):
    ...
    directories_found: int = Field(..., description="Number of directories found")
    total_size_bytes: int = Field(..., description="Total size in bytes")
    changed_from_previous: Optional[int] = Field(None, description="Number of changes from previous")
    is_baseline: bool = Field(..., description="Whether this is a baseline snapshot")
    
    # Tags
    tags: list[str] = Field(default_factory=list, description="Tags applied to snapshot")


class SnapshotEnvVarResponse(BaseSchema):
    """Environment variable response."""

    placeholder: str = Field(..., description="Environment variable placeholder")
...
class SnapshotDetailResponse(SnapshotSummary):
    ...
    working_directory: Optional[str] = Field(None, description="Working directory at scan time")
    
    # Relationships
    env_vars: list[SnapshotEnvVarResponse] = Field(default_factory=list, description="Environment variables")
    annotations: list[SnapshotAnnotationResponse] = Field(default_factory=list, description="Annotations")
    tag_details: list[SnapshotTagResponse] = Field(default_factory=list, description="Tag details")
...
class SnapshotStatsResponse(BaseSchema):
    ...
    oldest_snapshot: Optional[datetime] = Field(None, description="Oldest snapshot time")
    newest_snapshot: Optional[datetime] = Field(None, description="Newest snapshot time")
    average_files_per_snapshot: float = Field(..., description="Average files per snapshot")
    total_changes_tracked: int = Field(..., description="Total changes tracked")
