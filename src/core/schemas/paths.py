...
class PathBase(BaseSchema):
    ...
    path_template: str = Field(..., description="Path template with placeholders")
...
class PathResponse(PathBase):
    ...
    snapshot_id: int = Field(..., description="Snapshot ID this path belongs to")
    exists: bool = Field(..., description="Whether path exists")
    type: Optional[str] = Field(None, description="Path type (file, directory, symlink)")
...
class PathSummary(PathResponse):
    ...
    accessed_time: Optional[datetime] = Field(None, description="Last access time")
    permissions: Optional[str] = Field(None, description="File permissions")
    
    # Directory metadata
    item_count: Optional[int] = Field(None, description="Number of items in directory")
...
class FileContentResponse(BaseSchema):
    ...
    content_type: str = Field(..., description="Content type (json, text, binary, markdown)")
    size_bytes: int = Field(..., description="Content size in bytes")
    compression: str = Field(..., description="Compression type (none, gzip, zlib)")
    reference_count: int = Field(..., description="Number of paths referencing this content")
...
class JsonDataResponse(BaseSchema):
    ...
    json_path: str = Field(..., description="JSON path (e.g., 'mcpServers.server1.command')")
...
class McpServerResponse(BaseSchema):
    ...
    env: Optional[str] = Field(None, description="Environment variables (JSON object)")
...
class ClaudeConfigResponse(BaseSchema):
    ...
    num_startups: Optional[int] = Field(None, description="Number of startup configurations")
    ...
    largest_project_size: Optional[int] = Field(None, description="Largest project size")
...
class PathDetailResponse(PathSummary):
    ...
    # Related data
    content: Optional[FileContentResponse] = Field(None, description="File content")
    json_data: list[JsonDataResponse] = Field(default_factory=list, description="Parsed JSON data")
    claude_config: Optional[ClaudeConfigResponse] = Field(None, description="Claude config details")
    mcp_servers: list[McpServerResponse] = Field(default_factory=list, description="MCP servers")
    annotations: list["PathAnnotationResponse"] = Field(default_factory=list, description="Annotations")
...
class PathAnnotationResponse(BaseSchema):
    ...
    created_by: Optional[str] = Field(None, description="Who created the annotation")
