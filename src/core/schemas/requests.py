...
class SnapshotCreateRequest(BaseSchema):
    ...
    triggered_by: Optional[str] = Field(
        None, max_length=100, description="User or system triggering scan")
    ...
    include_content: bool = Field(
        default=True, description="Whether to capture file contents")
    compress_large_files: bool = Field(
        default=True, description="Whether to compress large files")
    detect_changes: bool = Field(
        default=True, description="Whether to detect changes from previous")
...
class SnapshotQueryRequest(PaginationParams):
    ...
    triggered_by: Optional[str] = Field(
        None, description="Filter by triggered by")
    os_type: Optional[str] = Field(None, description="Filter by OS type")
    is_baseline: Optional[bool] = Field(
        None, description="Filter by baseline status")
    has_changes: Optional[bool] = Field(
        None, description="Filter by whether snapshot has changes")
    ...
    # Tags
    tags: Optional[list[str]] = Field(
        None, description="Filter by tags (any match)")
    tags_all: Optional[list[str]] = Field(
        None, description="Filter by tags (all must match)")

    # Search
    search: Optional[str] = Field(
        None, description="Search in notes and triggered_by")
...
class PathQueryRequest(PaginationParams):
    ...
    snapshot_id: int = Field(..., ge=1,
                             description="Snapshot ID to query paths from")
    ...
    has_content: Optional[bool] = Field(
        None, description="Filter by whether content is captured")
    has_errors: Optional[bool] = Field(
        None, description="Filter by whether errors occurred")

    # Size filters
    min_size_bytes: Optional[int] = Field(
        None, ge=0, description="Minimum file size")
    max_size_bytes: Optional[int] = Field(
        None, ge=0, description="Maximum file size")

    # Time filters
    modified_after: Optional[str] = Field(
        None, description="Modified after timestamp")
    modified_before: Optional[str] = Field(
        None, description="Modified before timestamp")

    # Search
    search: Optional[str] = Field(
        None, description="Search in path names and templates")
...
class CompareSnapshotsRequest(BaseSchema):
    ...
    include_unchanged: bool = Field(
        default=False, description="Include unchanged paths")
    include_content_diff: bool = Field(
        default=False, description="Include content diffs for text files")
    max_diff_size: int = Field(
        default=1024 * 1024, description="Max file size for content diff (bytes)")

    # Filters
    categories: Optional[list[str]] = Field(
        None, description="Limit to specific categories")
    change_types: Optional[list[str]] = Field(
        None, description="Filter by change types")
...
class AnnotateSnapshotRequest(BaseSchema):
    ...
    annotation_type: Optional[str] = Field(
        None, max_length=20, description="Annotation type (note, warning, error)")
    created_by: Optional[str] = Field(
        None, max_length=100, description="Creator name")
...
class DeleteSnapshotRequest(BaseSchema):
    ...
    snapshot_ids: list[int] = Field(..., min_length=1,
                                    description="Snapshot IDs to delete")
    delete_orphaned_content: bool = Field(
        default=True, description="Delete orphaned file contents")
    vacuum_after: bool = Field(
        default=True, description="Vacuum database after deletion")
...
class ExportSnapshotRequest(BaseSchema):
    ...
    include_content: bool = Field(
        default=True, description="Include file contents")
    ...
    include_metadata: bool = Field(
        default=True, description="Include snapshot metadata")
